...
class SpecModel(SpecPrep):
    ...
    def toTensor(self):
        ...
        if self.sparse_op == False:
            ...
            self.Zar_re_indices = [
                tf.convert_to_tensor(x, tf.int64) for x in self.Zar_re_indices
            ]  # int64 required by tf.sparse.SparseTensor
            self.Zar_im_indices = [
                tf.convert_to_tensor(x, tf.int64) for x in self.Zar_im_indices
            ]
            self.Zar_re_values = [
                tf.convert_to_tensor(x, tf.float32) for x in self.Zar_re_values
            ]
            self.Zar_im_values = [
                tf.convert_to_tensor(x, tf.float32) for x in self.Zar_im_values
            ]

            self.Zar_size = tf.convert_to_tensor(self.Zar_size, tf.int64)

            self.Z_re = [
                tf.sparse.SparseTensor(x, y, self.Zar_size)
                for x, y in zip(self.Zar_re_indices, self.Zar_re_values)
            ]
            self.Z_im = [
                tf.sparse.SparseTensor(x, y, self.Zar_size)
                for x, y in zip(self.Zar_im_indices, self.Zar_im_values)
            ]
    ...
    def loglik_sparse(self, params):
        ...
        sum_xα_xβ_dot_y_real = [
            tf.sparse.sparse_dense_matmul(self.Z_re[i], tf.transpose(xα[:, i]))
            - tf.sparse.sparse_dense_matmul(
                self.Z_im[i], tf.transpose(xβ[:, i])
            )
            for i in range(self.num_obs)
        ]
        sum_xα_xβ_dot_y_imag = [
            tf.sparse.sparse_dense_matmul(self.Z_re[i], tf.transpose(xβ[:, i]))
            + tf.sparse.sparse_dense_matmul(
                self.Z_im[i], tf.transpose(xα[:, i])
            )
            for i in range(self.num_obs)
        ]

        u_re = self.y_re - tf.transpose(
            tf.stack(sum_xα_xβ_dot_y_real), [2, 0, 1]
        )
        u_im = self.y_im - tf.transpose(
            tf.stack(sum_xα_xβ_dot_y_imag), [2, 0, 1]
        )

        internal = (
            tf.multiply(tf.square(u_re) + tf.square(u_im), exp_xγ_inv),
        )
